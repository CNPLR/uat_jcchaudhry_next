...
export default function Subscribe() {
  ...
  const submit = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (validate.test(email)) {
      try {
        const res = await axios.post(`${path}subscribe`, {
          email,
          platform: "Desktop",
        });
      ...
      <div className="lg:flex justify-between mx-auto max-w-[1200px] w-full lg:h-28 py-5 lg:py-0 items-center">
        <div>
          <MainHeading
            ...
            mainHeading="Subscribe to Get Latest Updates"
          />
        </div>
        ...
        <div className="lg:flex justify-around items-center lg:space-x-8">
          <div className="text-center my-4 lg:my-0">
            <input
              ...
              className="text-center custom-input w-[300px] placeholterText h-12 outline-none rounded-md placeholder-gray-600"
          ...
          <div className="text-center">
            <button
              onClick={submit}
              className="transition buttonHover text-center shadow-md bg-orange-500 text-white font-semibold px-10 py-3 rounded-lg border"
            >
              Subscribe
            </button>
          </div>
